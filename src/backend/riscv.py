"""
RISC-V 64-bit backend (Week 10)
Generates GAS assembly for RV64GC Linux.

Stack frame:
    s0 = old sp (frame pointer)
    s0 - 8        : saved ra
    s0 - 16       : saved s0
    s0 - (16+k*8) : local slot k  (k = 1, 2, ...)

Calling convention: a0-a7 for args, a0 for return value.
Assemble:  riscv64-linux-gnu-as -o out.o out.s
Link:      riscv64-linux-gnu-gcc out.o -o program -lc
"""
...
# Binary arithmetic: IR opcode -> RISC-V mnemonic
_BINOP = {"ADD":"add", "SUB":"sub", "MUL":"mul", "DIV":"div", "MOD":"rem"}

# Comparison ops: each emits instructions that leave result in t0
# (after loading l->t0, r->t1)
_CMP = {
    "LT": ["slt   t0, t0, t1"],
    "GT": ["slt   t0, t1, t0"],
...
class RiscVBackend:
    def __init__(self, prog: IRProgram):
        self.prog   = prog
        self.out    = []          # accumulated output lines
        self.strs   = {}          # string_value -> data label
        self.scnt   = 0
    ...
    def _build_frame(self, func: IRFunction):
        ...
        slots[name] = raw offset (address = s0 - (offset + 16)).
        """
        slots, arrs, cur = {}, {}, 0
        ...
        for ins in func.instructions:
            ...
            if op == "ALLOC_ARRAY":
                arrs[a[0]] = cur + 8
                alloc(a[0], int(a[1]))
            elif op in ("CONST","LOAD","LOAD_ARR","READ_INT","ADD","SUB","MUL",
                        "DIV","MOD","NEG","INC","DEC","LT","LE","GT","GE",
                        "EQ","NE","AND","OR","NOT"):
                alloc(a[0])
            elif op == "STORE":
                alloc(a[0])
        ...
        fsize = (cur + 16 + 15) & ~15   # include 16 bytes for ra + s0
    ...
    def generate(self) -> str:
        ...
        for fn in self.prog.functions:
            for ins in fn.instructions:
                if ins.op == "CONST" and isinstance(ins.args[1], tuple):
                    if ins.args[1][0] == "string":
                        self._intern(ins.args[1][1])

        # Keep output assembly clean: no extra generator header comments.
        self.e()
        self.e("    .section .data")
    ...
    def _gen_func(self, func: IRFunction):
        slots, arrs, N = self._build_frame(func)
        self.fn = func.name
        r = lambda name: self.ref(slots, name)   # shorthand
        ...
        # move incoming args from registers to stack slots
        ...
        pend:  List[str]     = []   # accumulated PARAM args before a CALL

        for ins in func.instructions:
            op, a = ins.op, ins.args

            if   op == "FUNC_ENTRY": pass

            elif op == "LABEL":      self.e(f"{self.lbl(a[0])}:")
            elif op == "JMP":        self.i(f"j     {self.lbl(a[0])}")
            elif op == "JMP_IF":
                ...
                if kind in ("int","uint32","bool"):
                    ...
                    self.i(f"# float not supported, using 0")
            ...
            elif op == "ALLOC_ARRAY": pass

            elif op == "LOAD_ARR":
                dest, arr, idx = a
                self.i(f"ld    t1, {r(idx)}")
                self.i(f"slli  t1, t1, 3")
                self.i(f"addi  t0, s0, -{arrs[arr]+16}")
                self.i(f"sub   t0, t0, t1")
                self.i(f"ld    t0, 0(t0)")
                self.i(f"sd    t0, {r(dest)}")
                ...
                kinds[dest] = "bool"
                self.i(f"ld    t0, {r(l)}")
                self.i(f"sltu  t0, zero, t0")
                self.i(f"ld    t1, {r(rv)}")
                self.i(f"sltu  t1, zero, t1")
                self.i(f"and   t0, t0, t1")
                self.i(f"sd    t0, {r(dest)}")
                ...
                self.i(f"ld    t1, {r(rv)}")
                self.i(f"sltu  t1, zero, t1")
                self.i(f"or    t0, t0, t1")
                self.i(f"sd    t0, {r(dest)}")

            elif op == "NOT":
                kinds[a[0]] = "bool"
                self.i(f"ld    t0, {r(a[1])}")
                self.i(f"sltiu t0, t0, 1")
                self.i(f"sd    t0, {r(a[0])}")
